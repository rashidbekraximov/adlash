      ...
      <div class="table-responsive">
        <table class="table text-nowrap align-middle mb-0">
          ...
          <tr v-for="(t,index) in paginatedData" :id="index" v-bind:key="index">
            ...
            <td class="shadow-none lh-1 fw-medium text-black-emphasis">
              {{ $formatNumber(t.totalValue) }} SO'M
            </td>
            <td class="shadow-none lh-1 fw-medium text-black-emphasis">
              {{ t.location }}
            </td>
...
export default {
  ...
  methods :{
    async getOrderList() {
      try {
        this.orders = await this.$api.getDataList("orders" + localStorage.getItem("lang"));
      } catch (error) {
    ...
    async handleDelete(id) {
      await this.$delet.deleteRow("order/delete/" + id, this.getOrderList());
    }
  },
  computed: {
    totalPages() {
      return Math.ceil(this.orders !== undefined ? this.orders.length : 0 / this.pageSize);
    },
    paginatedData() {
      const startIndex = (this.currentPage - 1) * this.pageSize;
      const endIndex = startIndex + this.pageSize;
      return this.orders !== undefined ? this.orders.slice(startIndex, endIndex) : [];
    }
  }
};
</script>
