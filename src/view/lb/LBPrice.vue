<template>
  ...
  <CreateLBPriceModal   :edit-id="editId"/>
</template>

<script>
import PaginationCustom from "../../pages/PaginationCustom";
import CreateLBPriceModal from "@/view/lb/modal/CreateLBPriceModal";
import {checkPermission} from "@/message/message";

export default {
  // eslint-disable-next-line vue/multi-word-component-names
  name: "LBPrice",
  components:{PaginationCustom, CreateLBPriceModal},
  data() {
    return {
      prices: [],
      editId: 0,
  ...
  methods: {
    getPrices() {
      this.$http.get("lb-prices" + localStorage.getItem("lang")).then(res => {
