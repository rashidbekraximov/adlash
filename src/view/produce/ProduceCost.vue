<template>
  ...
  <CreateProduceCostModal :edit-id="editId"/>
</template>
...
import CreateProduceCostModal from "@/view/produce/modal/CreateProduceCostModal.vue";
import PaginationCustom from "@/pages/PaginationCustom.vue";
import {checkPermission} from "@/message/message";

export default {
  name: "ProduceCost",
  components: {PaginationCustom,CreateProduceCostModal},
  data() {
    return {
      costs:[],
      editId: 0,
  ...
  methods :{
    getCosts() {
      axios.get("costs" + localStorage.getItem("lang")).then(res => {
        this.costs = res.data
      }).catch((reason) => {
    ...
    deleteRow(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.get("cost/delete/" + id).then(res => {
