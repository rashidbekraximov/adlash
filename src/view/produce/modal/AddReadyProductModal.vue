            ...
            <div class="row">
              ...
              <div class="col-lg-4 col-md-6">
                <div class="form-group mb-15 mb-md-25">
                  ...
                  <select class="form-select fs-md-15 text-black shadow-none" not_empty='true' v-model="remainder.productTypeId">
              ...
              <div class="col-lg-6 col-md-12">
                <div class="form-group mb-15 mb-md-25">
                  <label class="d-block text-black fw-semibold mb-10">
                    1 dona mahsulotning sement sarfi
                  </label>
                  <input
                      type="number"
                      class="form-control shadow-none rounded-0 text-black"
                      placeholder="e.g. 100"
                      v-model="remainder.costPerKgSementAmount"
                      not_empty='true'
                  />
                </div>
              </div>
...
export default{
  name: "AddReadyProductModal",
  data() {
    return {
      remainder:{
        productTypeId : 0,
        costAmount: 0,
        costPerKgSementAmount: 0,
        amount: 0,
        date: '',
        sexEnum: 'SEX1'
      },
  ...
  methods: {
    save(){
      let valid = validator();
      if (valid) {
        axios.post("ready-product/save" + localStorage.getItem("lang"), this.remainder).then(res => {
          if (res.status === 201) {
    ...
    getById(id) {
      if (!isNaN(parseInt(id))){
        axios.get("remainder/" + parseInt(id)).then(res => {
