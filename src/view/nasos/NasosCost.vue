<template>
  ...
  <CreateNasosCostModal :edit-id="editId"/>
</template>
...
import CreateNasosCostModal from "./modal/CreateNasosCostModal";

export default {
  name: "NasosCost",
  components: {PaginationCustom,CreateNasosCostModal},
  data() {
    return {
      costs:[],
      editId: 0,
  ...
  methods :{
    getCosts() {
      this.$http.get("nasos-costs" + localStorage.getItem("lang")).then(res => {
        this.costs = res.data
      }).catch((reason) => {
    ...
    deleteRow(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.$http.get("nasos-cost/delete/" + id + localStorage.getItem("lang")).then(res => {
            if (res.status === 200){
