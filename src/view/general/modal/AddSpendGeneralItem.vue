<template>
  <div
      ...
      id="createSpendGeneralItemModal"
      tabindex="-1"
      aria-hidden="true"
  >
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content p-15 p-md-40">
        <div class="modal-header d-block ps-0 pe-0 pt-0 pb-15 pb-md-25">
          <h5 class="modal-title fw-bold text-black">Sarflash</h5>
        </div>
        <div class="modal-body ps-0 pe-0 pb-0 pt-15 pt-md-25">
          <form>
            <div class="row">
              ...
              <div class="col-lg-4 col-md-6" v-if="isSpare">
                <div class="form-group mb-15 mb-md-25">
                  <label class="d-block text-black fw-semibold mb-10">
                    Ehtiyot qism turi
                  </label>
                  <select class="form-select fs-md-15 text-black shadow-none" @change="getSparePartPrices(sparePart.sparePartTypeId)" not_empty='true' v-model="sparePart.sparePartTypeId">
                    <option selected disabled value="0">Tanlang...</option>
                    <option v-for="(u,index) in spareTypes" v-bind:key="index" :value="index">{{ u }}</option>
                  </select>
                </div>
              </div>
              <div class="col-lg-4 col-md-6" v-if="isFuel">
                <div class="form-group mb-15 mb-md-25">
                  <label class="d-block text-black fw-semibold mb-10">
                    Yoqilg'i turi
                  </label>
                  <select class="form-select fs-md-15 text-black shadow-none" not_empty='true' @change="getSparePartPrices(sparePart.fuelTypeId)" v-model="sparePart.fuelTypeId">
                    <option selected disabled value="0">Tanlang...</option>
              ...
              <div class="col-lg-4 col-md-6">
                <div class="form-group mb-15 mb-md-25">
                  ...
                  <select class="form-select fs-md-15 text-black shadow-none" not_empty='true' v-model="sparePart.price">
                    <option selected disabled value="0">Tanlang...</option>
                    <option v-for="(u,index) in prices" v-bind:key="index" :value="u.price">{{ u.price }}</option>
                  </select>
                </div>
              </div>
              <div class="col-lg-4 col-md-6">
                <div class="form-group mb-15 mb-md-25">
                  ...
                  <select class="form-select fs-md-15 text-black shadow-none" not_empty='true' v-model="sparePart.technicianId">
                    <option selected disabled value="0">Tanlang...</option>
  ...
  methods: {
    ...
    save() {
      ...
      if (valid) {
        ...
        axios.post("spending-spare-part/save" + localStorage.getItem("lang"), this.sparePart).then(res => {
          if (res.status === 201) {
